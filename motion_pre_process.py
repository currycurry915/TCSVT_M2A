...
def calculate_correlation_score_many_method(prompt, attn_map, mag, x, start, end, cur_step, output_folder):
    ...
    for p_idx in range(1, len(split_prompt)+1):
        ...
        # # 1. cv2.TM_SQDIFF 일치: 0 / 불일치 : 255
        # correlation_score = cv2.matchTemplate(frame_per_one_attention_np[:,:,p_idx], mag_np, cv2.TM_SQDIFF)
        # correlation_score_norm = 1 - (correlation_score/255)

        # # 2. cv2.TM_SQDIFF_NORMED / 위에 것을 0~1로 정규화
        ...
        # # 4. cv2.TM_CCORR_NORMED / 위에거 norm
        ...
        # # 7. ssim
        # attn = frame_per_one_attention_np[:,:,p_idx]
        # magnitude = mag_np
        # attn = np.expand_dims(attn,axis=2)
        # magnitude = np.expand_dims(magnitude,axis=2)
        # correlation_score_norm = ssim(org_img=attn, pred_img=magnitude)

        # # 8. SAM
        # correlation_score_norm = sam(org_img=frame_per_one_attention_np[:,:,p_idx], pred_img=mag_np)/255
        
        #9. MI
        # correlation_score_norm = skimage.metrics.normalized_mutual_information(frame_per_one_attention_np[:,:,p_idx], mag_np)

        # print(correlation_score_norm)
