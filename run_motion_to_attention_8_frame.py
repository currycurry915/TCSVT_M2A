            ...
            if is_cross or (self.num_self_replace[0] <= self.cur_step < self.num_self_replace[1]):
                ...
                if is_cross:
                    ...
                    for i in range (len(forward_list)+1):
                        if i == len(forward_list):
                            flow = -backward_list[-1]
                            flow = torch.tensor(flow).permute(2,0,1)
                            magnitude = motion_pre_process_24f.motion(w, flow)
                            magnitude = magnitude / magnitude.max()
                            attn_base_map[8*i:8*(i+1),:,:,:] = motion_pre_process_24f.calculate_correlation_score_many_method(prompt, attn_base_map[8*i:8*(i+1),:,:,:], magnitude, x, start, end, cur_step, output_folder)
                            attn_replace_map[8*i:8*(i+1),:,:,:] = motion_pre_process_24f.calculate_correlation_score_many_method(prompt, attn_replace_map[8*i:8*(i+1),:,:,:], magnitude, x, start, end, cur_step, output_folder)
                            save_image(magnitude.squeeze(-1), output_folder + '/'+'magnitude_'+str(i)+'.png')
                            attn_base_map[8*i:8*(i+1),:,:,start:end] = (magnitude.repeat(8, 1, 1, end-start))*x
                            attn_replace_map[8*i:8*(i+1),:,:,start:end] = (magnitude.repeat(8, 1, 1, end-start))*x
