...
def main(
        ...
        def forward(self, forward_list, backward_list, attn, is_cross: bool, place_in_unet: str):
            ...
            if is_cross or (self.num_self_replace[0] <= self.cur_step < self.num_self_replace[1]):
                ...
                if is_cross:
                    ...
                    # Load angle from vector.json
                    with open('vector.json', 'r') as f:
                        data = json.load(f)
                        angle_deg = data['angle_deg']

                    # Convert angle from degrees to radians and define range
                    angle_rad = np.deg2rad(angle_deg)
                    
                    angle_tolerance = np.deg2rad(30)  # Example: ±10 degrees

                    angle_min = angle_rad - angle_tolerance
                    angle_max = angle_rad + angle_tolerance
                    
                    for flow in [flow_0_1, flow_1_2, flow_2_3, flow_3_2]:
                        # Calculate angles of the flow vectors
                        calculated_angles = np.arctan2(flow[1], flow[0])  # Y, X to angle

                        # Create a mask for angles within the specified range
                        mask = (calculated_angles >= angle_min) & (calculated_angles <= angle_max)

                        # Apply mask: Set values outside the range to 0
                        flow[:, ~mask] = 0

                        # Calculate motion magnitude
                        flow_magnitude = motion_pre_process_24f.motion(w, flow)
                        flow_magnitude = flow_magnitude / flow_magnitude.max()
                        flow_magnitudes.append(flow_magnitude)

                    forward_flow_0_1_magnitude = flow_magnitudes[0]
                    forward_flow_1_2_magnitude = flow_magnitudes[1]
                    forward_flow_2_3_magnitude = flow_magnitudes[2]
                    backward_flow_3_2_magnitude = flow_magnitudes[3]
    ...
    # # image_all_attetion_map, attn_images = show_cross_attention(controller, 16, ["up", "down"])
    # tokens_for_attention = tokenizer.encode(prompts[0])
    # attention_maps = aggregate_attention(controller, 16, ["up", "down"], True, 0)
    # attn_maps_per_token = attention_maps[:, :, :len(tokens_for_attention)]
    # print(attn_maps_per_token.shape)
